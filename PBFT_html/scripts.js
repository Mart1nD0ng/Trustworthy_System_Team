(function(){
    ...
    function exec(){
      ...
      var m = parseInt($("#bft_m").val());   // 配置するビザンチンノード数
      var f = Math.floor((n - 1) / 3.0);      // 許容可能なビザンチンノード数
      var maliciousOrigin = $("#bft_faulty_proposer").prop("checked");
      var falsehoodMessage = $("#bft_allow_tampering").prop("checked");
      var w = Math.min(screen.width(), screen.height());  // 描画領域の大きさ
      var iw = 40;  // アイコンの縦横の大きさ
      var th = 12; // テキストの高さ
      var phase = 3;
      var phaseInterval = 2.2;
      var phaseFPS = 15.0;
      var phaseStep = phaseInterval * phaseFPS;
      ...
      // 正常なプロセスの全ての commit 結果が同一かを合意結果とする
      (function(){
        var truth = 0;
        var falsehood = 0;
        var rejected = 0;
      ...
      function drawForeground(phase){
        ...
        for(var i=0; i<xy.length; i++){
          ...
          function _draw(value){
            var t = text(value); // value == null? "😈": value>=0? "Confirmed": "Rejected";
            var thc = 8;
